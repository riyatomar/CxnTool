...
#=====================>>After updating spatial rule for r6 and k7p
from scripts.file_utils import update_cnx_value

def handle_spatial_relations(parser_output, new_entries, spatial_count, ner_output):
    # Extract NER annotations
    ner_annotations = {}
    if isinstance(ner_output, dict) and "data" in ner_output:
        sentence_annotations = ner_output["data"][0].get("annotation", [])
        if isinstance(sentence_annotations, list):
    ...
    # New Rule: Handling 'r6' relation with 'k7p' and 'main', including LOC check
    for i, item in enumerate(parser_output):
        if (
            item.get('pos_tag') in ['NN', 'NNP'] and 
            item.get('dependency_relation') == 'r6' and 
            ner_annotations.get(item.get('original_word')) == 'LOC'
        ):
            head_index = int(item.get('head_index', -1))
